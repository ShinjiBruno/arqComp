...
RUNTIME_ARGS=(--stop-time="${SIM_TIME}" --vcd="${VCD_WAVE_FILE}" --wave="${GHW_WAVE_FILE}")

if [ -f "${EXECUTABLE}" ]; then
    echo "Executando binário elaborado: ${EXECUTABLE}"
    "${EXECUTABLE}" "${RUNTIME_ARGS[@]}" || { echo "ERRO durante a simulação (binário .exe)"; exit 1; }
elif [ -f "${OUTPUT_DIR}/${TOP_ENTITY}" ]; then
    echo "Executando binário elaborado: ${OUTPUT_DIR}/${TOP_ENTITY}"
    "${OUTPUT_DIR}/${TOP_ENTITY}" "${RUNTIME_ARGS[@]}" || { echo "ERRO durante a simulação (binário sem extensão)"; exit 1; }
else
    echo "Binário não encontrado. Executando via 'ghdl -r'..."
    ghdl -r ${VHDL_STANDARD} --workdir="${OUTPUT_DIR}" "$TOP_ENTITY" "${RUNTIME_ARGS[@]}" || { echo "ERRO durante a simulação (ghdl -r)"; exit 1; }
fi
